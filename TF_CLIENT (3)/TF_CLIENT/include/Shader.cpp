...
void CObjectsShader::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	for (int j = 0; j < m_nObjects; j++)
	{
		...
		XMStoreFloat4x4(&pbMappedcbGameObject->m_xmf4x4World, XMMatrixTranspose(XMLoadFloat4x4(&m_ppObjects[j]->GetWMatrix())));
...
#endif         
				/*이거 잘몰겠*/
				pGameObject->SetWPosition(fxPitch*x, fyPitch*y, fzPitch*z);
...
{
	CIlluminatedTexturedShader::Render(pd3dCommandList);
...
{
	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255); //256의 배수
	m_pd3dcbGameObjects = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes * m_nTrees, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);
...
void CInstancingShader::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	//{
	//	CB_GAMEOBJECT_INFO *pbMappedcbGameObject = (CB_GAMEOBJECT_INFO *)((UINT8 *)m_pcbMappedGameObjects + (j * ncbElementBytes));
	//	XMStoreFloat4x4(&pbMappedcbGameObject->m_xmf4x4World, XMMatrixTranspose(XMLoadFloat4x4(&m_ppTrees[j]->GetWMatrix())));
	//}
}
...
void CInstancingShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	CreateCbvAndSrvDescriptorHeaps(pd3dDevice, pd3dCommandList, m_nTrees, 1); //쉐이더 리소스뷰도 6개가 필요
	CreateShaderVariables(pd3dDevice, pd3dCommandList);
	...
	for (int x = 0; x < m_nTrees; x++)
	{
		pRotatingObject = new BillboardObject();
		//pRotatingObject->SetMesh(0, pTreeMesh);
#ifndef _WITH_BATCH_MATERIAL
		//pRotatingObject->SetMaterial(pTreeMaterial);
#endif

		XMFLOAT3 randomPosition;
		randomPosition.x = rand() % 700 + 300;
		randomPosition.z = rand() % 1700 + 300;
		float fHeight = pTerrain->GetHeight(randomPosition.x, randomPosition.z);
		pRotatingObject->SetWPosition(randomPosition.x, fHeight + 30.0f, randomPosition.z);
		pRotatingObject->SetCbvGPUDescriptorHandlePtr(m_d3dCbvGPUDescriptorStartHandle.ptr + (::gnCbvSrvDescriptorIncrementSize *x));
		//m_ppTrees[x] = pRotatingObject;
	}
	//m_ppTrees[0]->SetMaterial(m_pMaterial);
	//m_ppTrees[0]->SetMesh(0, pTreeMesh);
}
