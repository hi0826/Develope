...
{   
	*issucc = false; 
	...
	// id , x, y, hp , mp , exp ,lev 
	SQLLEN L_ID,L_LEV,L_EXP,L_HP,L_MP,L_ATK,L_X,L_Y,L_Z;
	SQLINTEGER  LEV, EXP, HP, MP, ATK;
	...
	if (retcode != SQL_ERROR)
	{
		...
		retcode = SQLBindCol(hstmt, 2, SQL_C_LONG, &LEV, 100, &L_LEV);
		retcode = SQLBindCol(hstmt, 3, SQL_C_LONG, &EXP, 100, &L_EXP);
...
{
	WCHAR EXECORDER[256];
	wsprintf(EXECORDER, TEXT("EXEC dbo.SaveUserData %S,%d,%d,%d,%d,%d,%f,%f,%f "),
		user.GetName(),user.GetLEV(), user.GetEXP(), user.GetHP(), user.GetMP(), user.GetATK(),
