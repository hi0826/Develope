...
D3D12_SHADER_BYTECODE UIShader::CreateVertexShader(ID3DBlob **ppd3dShaderBlob, UINT nIndex)
{
	...
	if (nIndex == HPHEART)
		return(CompileShaderFromFile(L"UI.hlsl", "HPHEART_VS", "vs_5_1", ppd3dShaderBlob));
...
{
	//						개수
	m_pTexture = new CTexture(3, RESOURCE_TEXTURE2D, 0);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/WhiteBar.dds", 0);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/RedBar.dds", 1);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/RedHeart.dds", 2);
