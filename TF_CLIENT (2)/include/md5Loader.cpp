	...
	std::vector<std::wstring> texFileNameArray, float sx, float sy, float sz)
{
	...
	UINT*     pnIndices = NULL;
	XMFLOAT2* pxmf2UV = NULL;

	Model3D tempMD5Model;
	...
	if (fileIn)                                     // Check if the file was opened
	{
		while (fileIn)                              // Loop until the end of the file is reached
		{
			...
			{
				fileIn >> tempMD5Model.numJoints;        // Store number of joints
													 //fileIn >> mesh->NewMD5Model1.numJoints;
			}
			else if (checkString == L"numMeshes")
			{
				fileIn >> tempMD5Model.numSubsets;        // Store number of meshes or subsets which we will call them
													  //fileIn >> mesh->NewMD5Model1.numSubsets;
			}
			else if (checkString == L"joints")
			{
				...
				for (int i = 0; i < tempMD5Model.numJoints; i++)
				{
					...
					tempMD5Model.joints.push_back(tempJoint);    // Store the joint into this models joint vector
															 //mesh->NewMD5Model1.joints.push_back(tempJoint);
				}
			...
			else if (checkString == L"mesh")
			{
				...
				while (checkString != L"}")            // Read until '}'
				{
					...
					else if (checkString == L"numverts")
					{
						...
						pxmf3Positions = new XMFLOAT3[numVerts];
						pxmf2UV = new XMFLOAT2[numVerts];
						std::getline(fileIn, checkString);                // Skip rest of this line
						...
						for (int i = 0; i < numVerts; i++)
						{
							...
							fileIn >> tempVert.texCoord.x                // Store tex coords
								>> tempVert.texCoord.y;
							pxmf2UV[i].x = tempVert.texCoord.x;
							pxmf2UV[i].y = tempVert.texCoord.y;
				...
				for (int i = 0; i < subset.vertices.size(); ++i)
				{
					...
					for (int j = 0; j < tempVert.WeightCount; ++j)
					{
						...
						Joint tempJoint = tempMD5Model.joints[tempWeight.jointID];
						//Joint tempJoint = mesh->NewMD5Model1.joints[tempWeight.jointID];
						...
						//XMStoreFloat4(tempJointOrientation, )
				...
				for (int i = 0; i < subset.vertices.size(); i++)
				{
					...
					pxmf3Positions[i].x = subset.positions[i].x * sx;
					pxmf3Positions[i].y = subset.positions[i].y * sy;
					pxmf3Positions[i].z = subset.positions[i].z * sz;
				}
				...
				//mesh = new CMeshTextured(pd3dDevice, pd3dCommandList, numVerts, pxmf3Positions, pxmf2UV, subset.numTriangles * 3, pnIndices);
				//mesh->NewMD5Model1 = MD5Model;
				//mesh->NewMD5Model1.subsets.push_back(subset);
				mesh = new CMeshTextured(pd3dDevice, pd3dCommandList, numVerts, pxmf3Positions, pxmf2UV, subset.numTriangles * 3, pnIndices);
				MD5Model = tempMD5Model;
...
int UpdateMD5Model(CMesh*& p_Mesh, Model3D& MD5Model, float deltaTime, int animation)
{
	...
	for (int i = 0; i < MD5Model.animations[animation].numJoints; i++)
	{
		...
		interpolatedSkeleton.emplace_back(tempJoint);        // Push the joint back into our interpolated skeleton
	}

	for (int k = 0; k < 1; k++)
	{
		for (int t = 0; t < MD5Model.subsets[k].vertices.size(); ++t)
		{
			Vertex tempVert = MD5Model.subsets[k].vertices[t];
			//tempVert.pos = MD5Model.subsets[k].vertices[t].pos;
			tempVert.pos = XMFLOAT3(0, 0, 0);    // Make sure the vertex's pos is cleared first
			...
			for (int j = 0; j < tempVert.WeightCount; ++j)
			{
				...
				// Now move the verices position from joint space (0,0,0) to the joints position in world space, taking the weights bias into account
				tempVert.pos.x += (tempJoint.pos.x + rotatedPoint.x) * tempWeight.bias * 0.05;
				tempVert.pos.y += (tempJoint.pos.y + rotatedPoint.y) * tempWeight.bias * 0.05;
				tempVert.pos.z += (tempJoint.pos.z + rotatedPoint.z) * tempWeight.bias * 0.05;
		...
		//memcpy(&p_Mesh->m_pd3dVertexBuffer, &p_Mesh->NewMD5Model1.subsets[k].vertices[0], (sizeof(Vertex) * MD5Model.subsets[k].vertices.size()));
		p_Mesh->m_UploadBuffer->CopyData(0, MD5Model.subsets[k].vertices[0], p_Mesh->m_nVertices);
...
//int UpdateMD5Model(CMesh *& p_Mesh, vector<ModelAnimation>& MD5Model, float deltaTime, int animation)
//{
//
//	MD5Model[animation].currAnimTime += deltaTime;            // Update the current animation time
//
//	if (MD5Model[animation].currAnimTime > MD5Model[animation].totalAnimTime)
//	{
//		MD5Model[animation].currAnimTime = 0.0f;
//
//	}
//
//
//	// Which frame are we on
//	float currentFrame = MD5Model[animation].currAnimTime * MD5Model[animation].frameRate;
//	int frame0 = floorf(currentFrame);
//	int frame1 = frame0 + 1;
//	//int frame0 = 1;
//	//int frame1 = 1;
//	// Make sure we don't go over the number of frames    
//	if (frame0 == MD5Model[animation].numFrames - 1)
//		frame1 = 0;
//
//	float interpolation = currentFrame - frame0;    // Get the remainder (in time) between frame0 and frame1 to use as interpolation factor
//
...
//													// Compute the interpolated skeleton
//
//	for (int i = 0; i < MD5Model[animation].numJoints; i++)
//	{
//		Joint tempJoint;
//		Joint joint0 = MD5Model[animation].frameSkeleton[frame0][i];        // Get the i'th joint of frame0's skeleton
//		Joint joint1 = MD5Model[animation].frameSkeleton[frame1][i];        // Get the i'th joint of frame1's skeleton
//
//		tempJoint.parentID = joint0.parentID;                                            // Set the tempJoints parent id
//
//																						 // Turn the two quaternions into XMVECTORs for easy computations
...
//		for (int t = 0; t < MD5Model->subsets[k].vertices.size(); ++t)
//		{
//			Vertex tempVert = p_Mesh->NewMD5Model1.subsets[k].vertices[t];
//			//tempVert.pos = MD5Model.subsets[k].vertices[t].pos;
//			tempVert.pos = XMFLOAT3(0, 0, 0);    // Make sure the vertex's pos is cleared first
//			tempVert.normal = XMFLOAT3(0, 0, 0);    // Clear vertices normal
//
...
//			for (int j = 0; j < tempVert.WeightCount; ++j)
//			{
//				Weight tempWeight = p_Mesh->NewMD5Model1.subsets[k].weights[tempVert.StartWeight + j];
//				Joint tempJoint = interpolatedSkeleton[tempWeight.jointID];
//
//				// Convert joint orientation and weight pos to vectors for easier computation
//				XMVECTOR tempJointOrientation = XMVectorSet(tempJoint.orientation.x, tempJoint.orientation.y, tempJoint.orientation.z, tempJoint.orientation.w);
...
//			}
//
//			p_Mesh->NewMD5Model1.subsets[k].positions[t] = tempVert.pos;                // Store the vertices position in the position vector instead of straight into the vertex vector
//			p_Mesh->NewMD5Model1.subsets[k].vertices[t].normal = tempVert.normal;        // Store the vertices normal
//
//			XMStoreFloat3(&p_Mesh->NewMD5Model1.subsets[k].vertices[t].normal, XMVector3Normalize(XMLoadFloat3(&p_Mesh->NewMD5Model1.subsets[k].vertices[t].normal)));
//		}
//
//		// Put the positions into the vertices for this subset
//		for (int i = 0; i < p_Mesh->NewMD5Model1.subsets[k].vertices.size(); i++)
//		{
//
//			p_Mesh->NewMD5Model1.subsets[k].vertices[i].pos = p_Mesh->NewMD5Model1.subsets[k].positions[i];
//		}
//		//memcpy(&p_Mesh->m_pd3dVertexBuffer, &p_Mesh->NewMD5Model1.subsets[k].vertices[0], (sizeof(Vertex) * MD5Model.subsets[k].vertices.size()));
//		p_Mesh->m_UploadBuffer->CopyData(0, p_Mesh->NewMD5Model1.subsets[k].vertices[0], p_Mesh->m_nVertices);
//
//
//
//	}
