...
bool LoadIlluminatedMD5Model(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList, CMesh *& mesh, std::wstring filename, Model3D & MD5Model, std::vector<ID3D12Resource*>& shaderResourceViewArray, std::vector<std::wstring> texFileNameArray, float sx, float sy, float sz)
{
	XMFLOAT3* pxmf3Positions = NULL;
			...
			else if (checkString == L"mesh")
			{
				...
				while (checkString != L"}")            // Read until '}'
				{
					...
					else if (checkString == L"numverts")
					{
						fileIn >> numVerts;                                // Store number of vertices
						pxmf3Positions = new XMFLOAT3[numVerts];
						pxmf3Normals = new XMFLOAT3[numVerts];
						pxmf2UV = new XMFLOAT2[numVerts];
						std::getline(fileIn, checkString);                // Skip rest of this line
				...
				for (int i = 0; i < subset.vertices.size(); ++i)
				{
					...
					subset.vertices[i].normal.y = -XMVectorGetY(normalSum);
					subset.vertices[i].normal.z = -XMVectorGetZ(normalSum);
					pxmf3Normals[i] = subset.vertices[i].normal;
				...
				mesh = new CMeshIlluminatedTextured(pd3dDevice, pd3dCommandList, numVerts, pxmf3Positions, pxmf3Normals, pxmf2UV, subset.numTriangles * 3, pnIndices);
				//mesh = new CMeshTextured(pd3dDevice, pd3dCommandList, numVerts, pxmf3Positions, pxmf2UV, subset.numTriangles * 3, pnIndices);
				MD5Model = tempMD5Model;
				MD5Model.subsets.push_back(subset);
			}
...
int UpdateMD5Model(CMesh*& p_Mesh, Model3D& MD5Model, float deltaTime, int animation)
{
	...
	if (MD5Model.animations[animation].currAnimTime >= MD5Model.animations[animation].totalAnimTime) MD5Model.animations[animation].currAnimTime = 0.0f;
	...
	if (frame0 == MD5Model.animations[animation].numFrames - 1) frame1 = 0;
