...
void CScene::LoginSuccessPlayer(char * packet)
{
	...
	m_pPlayer->SetWPosition(Packet->pos);
	m_pPlayer->SetOOBB(Packet->pos);
	m_pPlayer->Rotate(Packet->dir);
	m_pPlayer->SetAnimState(Packet->state);
	m_pPlayer->SetCurMeshNum(Packet->character);
	m_pPlayer->SetStage(Packet->stage);
...
void CScene::PutPlayer(char * packet)
{
	...
	if (Packet->id < MAX_USER)
	{
		...
		m_pPlayerVector[ID]->SetCurMeshNum(Packet->character);
...
void CScene::RemovePlayer(char * packet)
{
	...
	if (Packet->id < MAX_USER)	m_pPlayerVector[Packet->id]->SetActive(false);
}
...
void CScene::PlayerHP(char * packet)
{
	sc_packet_damagestep_player* Packet = (sc_packet_damagestep_player*)packet;
	WORD ID = Packet->id;
	printf("Cur HP : %f\n", Packet->hp);
	m_pPlayerVector[ID]->SetHp(Packet->hp);
}

void CScene::PlayerChangeCharacter(char * packet)
{
	sc_packet_change_character* Packet = (sc_packet_change_character*)packet; 
	m_pPlayerVector[Packet->id]->SetCurMeshNum(Packet->num);
}

void CScene::PlayerCollision(char * packet)
{
	sc_packet_collision_player* Packet = (sc_packet_collision_player*)packet;
	m_pPlayerVector[Packet->id]->SetWPosition(Packet->pos);
}

void CScene::PlayerDead(char * packet)
{
	sc_packet_dead_player* Packet = (sc_packet_dead_player*)packet;
