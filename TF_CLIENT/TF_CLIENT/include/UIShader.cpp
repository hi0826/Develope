...
void UIShader::CreateStartShader(ID3D12Device * pd3dDevice, UINT nRenderTargets)
{
	m_nPipelineStates = 1;
	m_ppd3dPipelineStates = new ID3D12PipelineState*[m_nPipelineStates];

	CreateStartPSO(pd3dDevice, nRenderTargets);
}
...
void UIShader::CreateStartPSO(ID3D12Device * pd3dDevice, UINT nRenderTargets)
{
	ID3DBlob *pd3dVertexShaderBlob = NULL, *pd3dPixelShaderBlob = NULL;
	...
	::ZeroMemory(&d3dPipelineStateDesc, sizeof(D3D12_GRAPHICS_PIPELINE_STATE_DESC));
	d3dPipelineStateDesc.pRootSignature = m_pd3dGraphicsRootSignature;
	d3dPipelineStateDesc.VS = CreateVertexShaderStartScene(&pd3dVertexShaderBlob);
	d3dPipelineStateDesc.PS = CreatePixelShader(&pd3dPixelShaderBlob);
	d3dPipelineStateDesc.RasterizerState = CreateRasterizerState();
	d3dPipelineStateDesc.BlendState = CreateBlendState();
	d3dPipelineStateDesc.DepthStencilState = CreateDepthStencilState();
...
D3D12_SHADER_BYTECODE UIShader::CreateVertexShader(ID3DBlob **ppd3dShaderBlob, UINT nIndex)
{
	...
	if (nIndex == PANZA)
		return(CompileShaderFromFile(L"UI.hlsl", "PANZA_VS", "vs_5_1", ppd3dShaderBlob));
	if (nIndex == HPGAUGE)
		return(CompileShaderFromFile(L"UI.hlsl", "HPGAUGE_VS", "vs_5_1", ppd3dShaderBlob));
	if (nIndex == TREEICON)
		return(CompileShaderFromFile(L"UI.hlsl", "TREE_VS", "vs_5_1", ppd3dShaderBlob));
	if (nIndex == WARRIORICON)
		return(CompileShaderFromFile(L"UI.hlsl", "WARRIORICON_VS", "vs_5_1", ppd3dShaderBlob));
	if (nIndex == FAIRYICON)
		return(CompileShaderFromFile(L"UI.hlsl", "FAIRYICON_VS", "vs_5_1", ppd3dShaderBlob));
	if (nIndex == ENDING)
		return(CompileShaderFromFile(L"UI.hlsl", "ENDING_VS", "vs_5_1", ppd3dShaderBlob));
	if (nIndex == SANYANG)
		return(CompileShaderFromFile(L"UI.hlsl", "SANYANG_VS", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE UIShader::CreateVertexShaderStartScene(ID3DBlob ** ppd3dShaderBlob)
{
	return(CompileShaderFromFile(L"UI.hlsl", "STARTSCENE_VS", "vs_5_1", ppd3dShaderBlob));
}
...
void UIShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/PanZa.dds", 0);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/Teduri.dds", 1);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/HP.dds", 2);

	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/Tree.dds", 3);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/UI_WarriorIcon.dds", 4);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/UI_FairyIcon.dds", 5);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/GameOver.dds", 6);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/Sanyang.dds", 7);

	CreateSrvDescriptorHeaps(pd3dDevice, m_pTexture->GetTextureCount());
	CreateShaderVariables(pd3dDevice, pd3dCommandList);
	CreateShaderResourceViews(pd3dDevice, m_pTexture);
}
void UIShader::StartScene(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList)
{
	m_pTexture = new CTexture(1, RESOURCE_TEXTURE2D, 0);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/StartScene.dds", 0);
...
{
	XMFLOAT3 xmf3Position = m_pPlayer->GetWPosition();
...
void UIShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	...
	for (int i = 0; i < m_nPipelineStates; i++)
	{
		if (i == 6)
		{
			if (m_pPlayer->GetHp() > 10)
				continue;
		}
		if (i == 4 || i == 5 || i == 7)
		{
			if (m_pPlayer->GetCurMeshNum() == 0)
			{
				if (m_ppd3dPipelineStates)
					pd3dCommandList->SetPipelineState(m_ppd3dPipelineStates[4]);

				pd3dCommandList->SetGraphicsRootDescriptorTable(0, m_pTexture->GetArgumentInfos(4).m_d3dSrvGpuDescriptorHandle);

			}
			else if (m_pPlayer->GetCurMeshNum() == 1)
			{
				if (m_ppd3dPipelineStates)
					pd3dCommandList->SetPipelineState(m_ppd3dPipelineStates[5]);

				pd3dCommandList->SetGraphicsRootDescriptorTable(0, m_pTexture->GetArgumentInfos(5).m_d3dSrvGpuDescriptorHandle);

			}
			else if (m_pPlayer->GetCurMeshNum() == 2)
			{
				if (m_ppd3dPipelineStates)
					pd3dCommandList->SetPipelineState(m_ppd3dPipelineStates[7]);

				pd3dCommandList->SetGraphicsRootDescriptorTable(0, m_pTexture->GetArgumentInfos(7).m_d3dSrvGpuDescriptorHandle);
...
void UIShader::RenderStartScene(ID3D12GraphicsCommandList * pd3dCommandList, CCamera * pCamera)
{
	if (m_pd3dGraphicsRootSignature)
		pd3dCommandList->SetGraphicsRootSignature(m_pd3dGraphicsRootSignature);
	...
	pd3dCommandList->SetGraphicsRootDescriptorTable(0, m_pTexture->GetArgumentInfos(0).m_d3dSrvGpuDescriptorHandle);
