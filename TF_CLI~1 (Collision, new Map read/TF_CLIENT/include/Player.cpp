...
{   
	...
	//LoadMD5Model(pd3dDevice, pd3dCommandList, FairyMesh, L"Assets/Model/FairyMesh0724.md5mesh", m_Models[1] , shaderResourceViewArray1, texFileNameArray1, 1, 1, 1);
	//m_WarriorModel = tempOB[0].Model;
...
void CPlayer::Update(float fDeltaTime)
{
	...
	if (m_ppMeshes[i] != NULL)  UpdateMD5Model(m_ppMeshes[i], m_Models[0], fDeltaTime, m_AnimState);
	
	if (m_AnimState == MOVESTATE) {
		Rotate(m_Direction);
		CMoveObject::Move(Vector3::Add(XMFLOAT3(0, 0, 0), GetLook(), fDeltaTime * 50));
	}
	else if (m_AnimState == ATTACKSTATE) {
		if ((m_Models[0].animations[ATTACKSTATE].currAnimTime == 0.f)) m_AnimState = IDLESTATE;
	}
