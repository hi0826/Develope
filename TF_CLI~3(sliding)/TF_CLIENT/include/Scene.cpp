...
void CScene::ReleaseObjects()
{
	...
	if (m_pPlayerVector.size() > 0) {
		Safe_Release_VecList(m_pPlayerVector);
...
void CScene::PutPlayer(char * packet)
{
	...
	{
		PLAYERID = Packet->id;
		CNetWorkManager::GET_SINGLE()->SetNetworkID(Packet->id);
		FirstTime = false; 
	}
	...
	if (Packet->id == PLAYERID)
	{
		...
		m_pPlayer->SetDirection(Packet->dir);
		m_pPlayer->SetActive(true);

		m_pCamera->SetTarget(m_pPlayer);
		m_pCamera->SetPosition(Packet->pos);
	}
	else if (Packet->id <MAX_USER) 
	{   
		m_pPlayerVector[Packet->id]->SetAnimState(Packet->state);
		m_pPlayerVector[Packet->id]->SetWPosition(Packet->pos);
		m_pPlayerVector[Packet->id]->Rotate(Packet->dir);
		m_pPlayerVector[Packet->id]->SetActive(true);
	}
...
void CScene::RemovePlayer(char * packet)
{
	...
	if (Packet->id == PLAYERID) { m_pPlayer->SetActive(false); }
	else { m_pPlayerVector[Packet->id]->SetActive(false); }
}
...
void CScene::PlayerMove(char * packet)
{
	...
	{
	    m_pPlayerVector[Packet->id]->SetDirection(Packet->dir);
	    m_pPlayerVector[Packet->id]->SetWPosition(Packet->pos);
		m_pPlayerVector[Packet->id]->SetAnimState(Packet->state);
	}
}

void CScene::PlayerAttackHeat(char * packet)
{
	sc_packet_player_attack_heat Packet;
}

void CScene::PlayerAttackNoHeat(char * packet)
{
	sc_packet_player_attack_no_heat* Packet = (sc_packet_player_attack_no_heat*)packet;
	m_pPlayerVector[Packet->id]->SetAnimState(ATTACKSTATE);
}

void CScene::PlayerDie(char * packet)
{
}
