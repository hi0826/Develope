...
DayForestScene::DayForestScene()
{
	...
	Processor[SC_PLAYER_ATTACK_HEAT]    = &CScene::PlayerAttackHeat;
	Processor[SC_PLAYER_ATTACK_NO_HEAT] = &CScene::PlayerAttackNoHeat;
...
void DayForestScene::BuildObjects(ID3D12Device * pd3dDevice, ID3D12GraphicsCommandList * pd3dCommandList)
{
	...
	for (int i = 0; i < m_nTrees; ++i) {
		...
		m_pTrees[i]->SetWPosition(XMFLOAT3((m_PosTree[i].x - m_nWidthHalf) * 20 , 0, (m_PosTree[i].z - m_nHeightHalf) * 20));
	...
	for (int i = 0; i < 2; ++i) {
		...
		STONEMONS[i]->SetWPosition(150.f + 10*i, 0.f, 120.f);
...
void DayForestScene::Update(float fDeltaTime)
{
	...
	for (int i = 0; i < 2; ++i) {
		...
		case INTERSECTS:
			...
			PlayerPos = Vector3::Normalize(Vector3::Subtract(STONEMONS[i]->GetWPosition(), m_pPlayer->GetWPosition()));

			if (Vector3::CrossProduct(PlayerPos, m_pPlayer->GetLook()).y > 0)
			{
				XMFLOAT3 RightVector = m_pPlayer->GetRight();
				RightVector.x = RightVector.x * 3;
				RightVector.y = RightVector.y * 3;
				RightVector.z = RightVector.z * 3;
				PlayerLook = Vector3::Add(m_pPlayer->GetLook(), RightVector);
			...
			else
			{
				...
				RightVector.y = RightVector.y * 3;
				RightVector.z = RightVector.z * 3;
				PlayerLook = Vector3::Subtract(m_pPlayer->GetLook(), RightVector);
